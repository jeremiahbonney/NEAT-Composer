...
class Song:  # Want to incorperate a class which combines genotype (network), phenotype(song), and place in gen/gen number/fitness/etc... 
  ...
  gen_num = 0
  net_num = 0
  network = NEAT.GetGenomeList(pop)[0]
...
def gen_Song(genome, phenome): #Generates song based on output of a neural network    def gen_Song(genome, phenome) 
  global song_pop
  ...
  while(time <= 0.999999):
    ...
    time = time + (1.0/16.0)
  phenome.song = track 
  song_pop.append(phenome)   #phenome.song = track
...
def print_pop(phenome): #This print method sucks, find one that prints in straight line     If class implimented rename to print_Song
                             #If class implimented, kill this since it'll print only 1 song
  print phenome.net_num                                                
  for y in range(0,4): 
...
def main():
  ...
  for genomes in genome_list:
    gen_Song(genomes, Song(Track(), genome_list[x], 1, x+1))
    x = x+1
  while(programrun == 1):
    ...
    if(choice == "1"): #Print population
      ...
      for y in range(0, int(choice)-1):
        x = 0
        for genomes in genome_list:
          genomes.SetFitness(evaluate(song_pop[x].song))
          x=x+1
